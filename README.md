# Real Art AI backend (stable-diffusion-api-wrapper)

Flask API written to send back a json response containing txt2img base64 output through URL query using AUTOMATIC1111's [stable-diffusion-webUI text2img API](https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/API).

Allows clients to request images through GET requests supporting concurrent streams and directing the image queue allowing the request stack to always be fufilled. Also allows for custom networking layer params to be set.
